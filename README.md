...
![Kairos landing experience](docs/assets/kairos-landing.jpg)

**Showcase one-liner:** Kairos auctions temporary Uniswap v4 app-fee rights through sealed bids.
...
AI assistance and reused/reference material are disclosed in [AI_USAGE.md](docs/AI_USAGE.md). Kairos is released under the [MIT License](LICENSE).

## Local demo workstation

The React/Vite app in [app/](app/) connects to an injected wallet, guards for Unichain Sepolia, reads and validates the deployed auction/executor/hook wiring, and links each configured contract to Uniscan. It has explicit-wallet-confirmed, exact-allowance controls for approval, commit, reveal and refund, as well as an AES-GCM encrypted local bid vault. A separate deployer-gated panel creates fixed schedules and collects proceeds. It detects an ERC-2612-capable token plus permit-aware auction before showing exact 15-minute signature authorizations; plain approval remains the active fallback for the current deployment. Its read-only evidence dashboard shows the live auction phase, fixed-window countdown, counts, winner/right state, treasury/refund values, recent events and the remaining two-bidder rehearsal steps. An editable value calculator models captured volume, waived surcharge, bid and gas to show net value plus break-even flow and volume sensitivity; it is not financial advice, an execution quote or a prediction. The older receipt ledger remains a local simulator; it is not an event indexer.
