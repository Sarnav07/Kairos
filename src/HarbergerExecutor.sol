...
/// @notice Exact-input executor authenticated by a single Harberger lease right.
/// @dev Eligibility is transaction scoped: the hook cannot retain a waiver outside `unlockCallback`.
contract HarbergerExecutor is IFeeDiscountEligibility, IUnlockCallback, ReentrancyGuard {
    ...
    IFeeRightSource public immutable rightSource;
    ...
    event LeaseSwapExecuted(
        address indexed holder, PoolId indexed poolId, bool discounted, uint256 output
    );

    constructor(IPoolManager manager_, IFeeRightSource rightSource_, PoolId poolId_) {
        if (
            address(manager_).code.length == 0 || address(rightSource_).code.length == 0
                || PoolId.unwrap(poolId_) == bytes32(0)
        ) revert InvalidConfiguration();
        manager = manager_;
        rightSource = rightSource_;
        poolId = poolId_;
    }

    /// @notice All traders may swap; only the current solvent lease holder receives the waiver.
    function swap(
        ...
        if (PoolId.unwrap(poolId) != PoolId.unwrap(key.toId())) revert WrongPool();
        PFDAFeeHook hook = PFDAFeeHook(address(key.hooks));
        if (
            address(hook.manager()) != address(manager)
                || address(hook.eligibility()) != address(this)
        ) revert WrongHook();
        bool discounted = rightSource.activeHolder(poolId) == msg.sender;
        ...
        emit LeaseSwapExecuted(msg.sender, poolId, discounted, output);
    }

    function isEligible(PoolId queryPoolId, address executor) external view returns (bool) {
        return executor == address(this) && discountActive
            && PoolId.unwrap(currentPool) == PoolId.unwrap(queryPoolId);
    }
